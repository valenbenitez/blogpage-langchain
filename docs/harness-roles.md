...
2. **Tomá** una tarea `Not started` de la base de Notion. Cambiá su estado a `In progress`.
...
5. **Escribí tests** que validen los criterios de aceptación. **Es obligatorio cuando exista framework de test — no podés terminar sin tests en ese caso.**
6. **Verificá** ejecutando `./init.sh` (y `npm test` si existe). Si fallan → arreglá y volvé al paso 4.
...
- **Toda función nueva debe tener su test** (cuando el repo ya tenga runner de tests).
...
3. Para cada archivo modificado:
   ...
   - ¿Tiene su test correspondiente? (si aplica)
